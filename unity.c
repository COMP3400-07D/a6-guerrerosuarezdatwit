...
static const char UNITY_PROGMEM UnityStrElement[]                = " Array[";
                            ...
                            const UNITY_LINE_TYPE lineNumber)
{
    ...
    {
        UnityTestResultsFailBegin(lineNumber);
        UnityPrintExpectedAndActualStringsLen(expected, actual, i);
        UnityAddMsgIfSpecified(msg);
        UNITY_FAIL_AND_BAIL;
    }
}
                               ...
                               const UNITY_LINE_TYPE lineNumber)
{
    ...
    if (expected && actual)
    {
        ...
        /* fail if expected has ended but actual has not */
        if ((!expected[i]) && (actual[i]))
        {
            Unity.CurrentTestFailed = 1;
        }

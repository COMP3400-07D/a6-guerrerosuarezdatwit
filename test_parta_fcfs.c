...
void test_fcfs5(void) {
    // When
    procs = init_procs((int[]){5}, 1);
    TEST_ASSERT_NOT_NULL(procs);
    int total_time = fcfs_run(procs, 1);
...
void test_fcfs58(void) {
    // When
    procs = init_procs((int[]){5, 8}, 2);
    TEST_ASSERT_NOT_NULL(procs);
    int total_time = fcfs_run(procs, 2);
...
void test_fcfs582(void) {
    // When
    procs = init_procs((int[]){ 5, 8, 2 }, 3);
    TEST_ASSERT_NOT_NULL(procs);
    int total_time = fcfs_run(procs, 3);
    ...
    TEST_ASSERT_EQUAL_INT(13, procs[2].wait);
...
int main(void)
{
    ...
    RUN_TEST(test_fcfs5);
    RUN_TEST(test_fcfs58);
    RUN_TEST(test_fcfs582);
